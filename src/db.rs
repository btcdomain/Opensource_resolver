...
fn get_pool() -> Pool {
    ...
    if pool.is_ok() {
        ...
        warn!("Connect to database failed: {:?}", pool.err());
        get_pool()
    }
}
...
pub fn update_inscribe_info(id: u64, owner_address: &str) -> Result<()>{
    let mut conn = POOL.get_conn().unwrap();
    let sql = format!("UPDATE domain_inscription_info SET address = '{}', update_time = {} WHERE id = {}", owner_address, get_now_time(), id);
    info!("sql: {}", sql);
...
pub fn update_inscribe_info_update_time(id: u64) -> Result<()>{
    let mut conn = POOL.get_conn().unwrap();
    let sql = format!("UPDATE domain_inscription_info SET update_time = {} WHERE id = {}", get_now_time(), id);
    info!("sql: {}", sql);
    let x = conn.query_drop(sql);
    info!("update_inscribe_info_update_time: {:?}", x);
    Ok(())
}
